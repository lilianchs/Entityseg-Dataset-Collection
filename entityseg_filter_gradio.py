...
class EntitySegAnnotator:
    ...
    def load_json_data(self, json_path, image_dir, save_dir, discard_dir):
        ...
        # Get already processed files from both save and discard directories (avoid redundancy)
        self.processed = set()
    ...
    def display_current_image(self):
        ...
        if not self.current_batch or self.current_idx >= len(self.current_batch):
            return None, None
    ...
    def handle_click(self, evt: gr.SelectData):
        ...
        if self.segment_map is None:
            return self.display_current_image()[1]

        x, y = evt.index
        segment_idx = self.segment_map[y, x] # at clicked position
    ...
    def undo_last_deletion(self):
        ...
        if not self.deletion_history:
            return "No deletions to undo", self.display_current_image()[1]

        # Get the last deleted segment ID + remove
...
with gr.Blocks(title="EntitySeg Annotation Tool") as app:
    ...
    # Bottom panel showing batch preview (implement?)
